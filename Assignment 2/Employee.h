...
/*class Employee{
  protected:
    ...
    double healthcare_amount;
    int hours_worked;
    double hourly_rate;

  public:
    //Employee();
    Employee(int hw, double vd);
    Employee(double hr, int hw);
    double weekly_salary();
    //virtual double calculate_vacation_days() const = 0;
    //virtual double healthcare_contribution() const = 0;
};
...
class Professional : public Employee{
  ...
  public:
    Professional();
    Professional(double ms, int hw, double vd);
    void calculate_hourly_rate();
    double calculate_vacation_days();
